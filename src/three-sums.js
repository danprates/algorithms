  ...
  for (let current = 0; current < numbers.length; current++) {
    ...
    while (left < right) {
      ...
      if (sum < target) {
        ...
        result.push([numbers.at(current), numbers.at(left), numbers.at(right)].sort((a, b) => a - b))
        left++
        right--
      }
    }
...
export default () => {
  // should return correct results
  {
    ...
    deepEqual(solution1([1, 2, 3, 4, 5], 9), [[1, 3, 5], [2, 3, 4]])
    deepEqual(solution1([1, 2, 3, 4, 5], 100), [])
    deepEqual(solution1([], 5), [])
    deepEqual(solution1([1], 1), [])

    deepEqual(solution2([12, 3, 1, 2, -6, 5, -8, 6], 0), [[-8, 2, 6], [-8, 3, 5], [-6, 1, 5]])
    deepEqual(solution2([0, 0, 0], 0), [[0, 0, 0]])
    deepEqual(solution2([1, 2, 3, 4, 5], 9), [[1, 3, 5], [2, 3, 4]])
